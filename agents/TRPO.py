...
def mlp(x, hidden_layers, output_size, activation=tf.tanh, last_activation=None):
    '''
...
def TRPO(env_name, hidden_sizes=[32], cr_lr=5e-3, num_epochs=50, gamma=0.99, lam=0.95, number_envs=1, 
    ...
    p_means = mlp(obs_dim[0], hidden_sizes, act_dim, tf.tanh, last_activation=tf.tanh)
    log_std = tf.Variable(np.zeros(act_dim, dtype=np.float32) - 0.5)
    ...
    for ep in range(num_epochs):
        ...
        old_p_log = gaussian_log_likelihood(act_batch, p_means, log_std)
        old_p_means = p_means()
        old_log_std = log_std()

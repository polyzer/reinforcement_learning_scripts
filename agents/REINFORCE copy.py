...
def mlp(x, hidden_layers, output_size, activation=tf.nn.relu, last_activation=None):
    ...
    '''
    # pdb.set_trace()
    x = tf.keras.Input(shape=(None, x[0]))
    for l in hidden_layers:
        x = tf.keras.layers.Dense(units=l, activation=activation)(x)
    x = tf.keras.layers.Dense(units=output_size, activation=last_activation)(x)
    return tfp.layers.OneHotCategorical(x)
